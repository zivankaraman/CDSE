...
for (i in 1:12) {
    ...
    title(main = paste("Bois de Vincennes NDVI\n", lubridate::month(day, label = TRUE, abbr = FALSE, locale = "C"), 2022),
          cex.main = 0.8, col = fg, line = -3, outer = FALSE)
    terra::north(xy = "topleft", type = 2, col = "white", xpd = TRUE, cex = 0.8, label = "N", adj = c(0.55, 0.2))
    terra::sbar(d = 1, xy = "bottomleft", type = "bar", below = "m", labels = c(0, 500, 1000),
...
# save to disk
magick::image_write(image = img_animated, path = "Vincennes.gif")
